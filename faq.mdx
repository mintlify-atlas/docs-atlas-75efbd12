...
<AccordionGroup>
  ...
  <Accordion title="What commission formats are supported?">
    ...
    **Callable:** A function that receives `(price, size)` and returns the total commission for that fill.

    ```python
    def my_commission(price, size):
        return max(1.0, abs(size) * price * 0.001)
  ...
  <Accordion title="Can I use multiple time frames?">
    Yes. Use `backtesting.lib.resample_apply()` to compute an indicator on a higher time frame and align it back to the base data.
    ...
    class MultiTfStrategy(Strategy):
        def init(self):
            ...
            self.weekly_sma = self.I(
                resample_apply,
                'W',       # resample rule
                SMA,       # function to apply on resampled data
                self.data.Close,
                20,
                plot=True,
            )
  ...
  <Accordion title="Why is my optimization slow?">
    ...
    **Use model-based (SAMBO) optimization** for large search spaces:
    ...
    Optimization runs in parallel by default using `joblib`. On Windows, make sure your script has a `if __name__ == '__main__':` guard to avoid multiprocessing issues:
